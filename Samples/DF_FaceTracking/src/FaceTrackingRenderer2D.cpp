...
void FaceTrackingRenderer2D::DrawExpressions(PXCFaceData::Face* trackedFace, const int faceId)
{
	...
	static std::vector<std::vector<int>> intens;
	static std::vector<std::vector<std::string>> emot;
	

	static std::vector<int> ex,p_frame;
	static std::vector<std::string> ex_str;
	std::string wsstr;
	i_count++;

	//pi.push_back(i_count);
	std::ofstream out;          // поток для записи
	out.open("face_expression2d.txt");
	out << "frame number " << i_count << std::endl;
	...
	for (auto expressionIter = m_expressionMap.begin(); expressionIter != m_expressionMap.end(); expressionIter++)
	{
		...
		if (expressionsData->QueryExpression(expressionIter->first, &expressionResult))
		{
			...
			TextOut(dc2, xStartingPosition, yPosition, tempLine, (int)std::char_traits<wchar_t>::length(tempLine));
			ex.push_back(intensity);
			std::string w(expressionName.begin(), expressionName.end());
			ex_str.push_back(w);
			//p_frame.push_back(i_count);
			//out << w <<" "<< intensity <<std::endl;
			yPosition += rowMargin;
		}
	}

	emot.push_back(ex_str);
	intens.push_back(ex);
	...
	{
		out << "frame number " << i << std::endl;
		for (int j = 0; j < emot[0].size(); ++j)
		{

			out << emot[i][j] << " " << intens[i][j] << std::endl;
		}
...
void FaceTrackingRenderer2D::DrawPoseAndPulse(PXCFaceData::Face* trackedFace, const int faceId)
{
	...
	// write
	static int i_count = 0;
	//static std::vector<std::vector<int>> px, py, pi;
	static std::vector<int> pose_v1, pose_v2,pose_v3,p_frame;
	i_count++;
	
	//pi.push_back(i_count);
	std::ofstream out;          // поток для записи
	out.open("face_pose2d.txt");
	out << "frame number " << i_count << std::endl;
	...
	if (poseAnglesExist)
	{
		...
		p_frame.push_back(i_count);
		pose_v1.push_back(angles.yaw);
		pose_v2.push_back(angles.pitch);
		pose_v3.push_back(angles.roll);
	}	
	else
	{
		SetTextColor(dc2, RGB(255, 0, 0));	
	...
	{
		out << "frame number " << p_frame[i] << std::endl;
		out << "Yaw "  << pose_v1[i] << std::endl;
		out << "Pitch " << pose_v2[i] << std::endl;
		out << "Roll " << pose_v3[i] << std::endl;
	}
...
void FaceTrackingRenderer2D::DrawLandmark(PXCFaceData::Face* trackedFace)
{
	...
	static int i_count = 0;
	static std::vector<std::vector<int>> px,py,pi;
	static std::vector<int> pXX, pYY;
	i_count++;
	//pi.push_back(i_count);
	std::ofstream out;          // поток для записи
	out.open("face_landmarks2d.txt");
	out << "frame number " <<i_count << std::endl;
	...
	pXX.clear();
	pYY.clear();
	for (int i_c = 0; i_c < px.size(); ++i_c)
	{
		out << "frame " << i_c << std::endl;
		for (int i = 0; i < px[i_c].size(); ++i)
		{
			//out << "frame number " << pi[i] << std::endl;
			out << "X" << i << " " << px[i_c][i] << std::endl;
			out << "Y" << i << " " << py[i_c][i] << std::endl;
		}
	}
